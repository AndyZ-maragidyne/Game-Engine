...
void processMessages(std::string messageFromClient, std::vector<std::unique_ptr<GameObject>>* entityList, int64_t frame_delta, Timeline* t, EventManager* m) {
    ...
    if (foundIt == activeClientList.end()) {
        ...
        auto newEntity = std::make_unique<GameObject>(500, 200, 50, 50, clientID, true, true);
        newEntity->setPlayerNum(1);
        ...
        PhysicsComponent* p = new PhysicsComponent(newEntity.get(), 3, entityList, &frame_delta);
        p->setXGravity(3);
        newEntity->addComponent(p);
