...
EventManager* setup(int64_t* frame_delta) {
	...
	auto e1 = std::make_unique<GameObject>(1000, 0, 50, 200);
	e1->addComponent(new DrawComponent(e1.get()));


	//first floor
	auto e2 = std::make_unique<GameObject>(1000, 500, 50, 1000);
	e2->addComponent(new DrawComponent(e2.get()));

	//second floor
	auto e3 = std::make_unique<GameObject>(2000, 0, 50, 700);
	e3->addComponent(new DrawComponent(e3.get()));

	//second floor
	auto e4 = std::make_unique<GameObject>(2000, 1000, 50, 1000);
	e4->addComponent(new DrawComponent(e3.get()));

	//large platform after moving platform
	auto e5 = std::make_unique<GameObject>(3000, 0, 50, 400);
	e5->addComponent(new DrawComponent(e5.get()));
	//top wall
	auto e6 = std::make_unique<GameObject>(3000, 700, 50, 1000);
	e6->addComponent(new DrawComponent(e6.get()));
	//bottom wall
	auto e7 = std::make_unique<GameObject>(4000, 0, 50, 500);
	e7->addComponent(new DrawComponent(e7.get()));
	//bottom platform
	auto e8 = std::make_unique<GameObject>(4000, 800, 50, 1000);
	e8->addComponent(new DrawComponent(e8.get()));
	//middle platform
	auto e9 = std::make_unique<GameObject>(5000, 0, 50, 100);
	e9->addComponent(new DrawComponent(e9.get()));

	//roof left
	auto e12 = std::make_unique<GameObject>(5000, 400, 50, 1000);
	e12->addComponent(new DrawComponent(e12.get()));

	auto e13 = std::make_unique<GameObject>(6000, 0, 50, 300);
	e13->addComponent(new DrawComponent(e13.get()));
	//roof left
	auto e14 = std::make_unique<GameObject>(6000, 600, 50, 100);
	e14->addComponent(new DrawComponent(e14.get()));
	auto e15 = std::make_unique<GameObject>(6000, 1000, 50, 1000);
	e15->addComponent(new DrawComponent(e15.get()));


	auto e16 = std::make_unique<GameObject>(7000, 0, 50, 700);
	e16->addComponent(new DrawComponent(e16.get()));
	auto e17 = std::make_unique<GameObject>(7000, 1000, 50, 1000);
	e17->addComponent(new DrawComponent(e17.get()));

	auto e18 = std::make_unique<GameObject>(8000, 0, 50, 300);
	e18->addComponent(new DrawComponent(e18.get()));
	auto e19 = std::make_unique<GameObject>(8000, 600, 50, 1000);
	e19->addComponent(new DrawComponent(e19.get()));

	auto e20 = std::make_unique<GameObject>(9000, 0, 50, 450);
	e20->addComponent(new DrawComponent(e20.get()));
	auto e21 = std::make_unique<GameObject>(9000, 750, 50, 1000);
	e21->addComponent(new DrawComponent(e21.get()));

	auto e22 = std::make_unique<GameObject>(10000, 0, 50, 225);
	e22->addComponent(new DrawComponent(e22.get()));
	auto e23 = std::make_unique<GameObject>(10000, 525, 50, 1000);
	e23->addComponent(new DrawComponent(e23.get()));

	auto e10 = std::make_unique<GameObject>(500, -1000, 50000, 3000);
	auto e11 = std::make_unique<GameObject>(-500, -1000, 500, 3000);
	e10->addComponent(new DrawComponent(e10.get(), 0x00, 0x00, 0xFF, 0x00));
	auto sidescrollComponent = new SidescrollComponent(e10.get(), e11.get(), entityList, invisibleEntityList, frame_delta);
	sidescrollComponent->setScroll(-10);
	e10->addComponent(sidescrollComponent);

	
	e11->addComponent(new DrawComponent(e11.get(), 0x00, 0x00, 0xFF, 0x00));
	auto sidescrollComponent2 = new SidescrollComponent(e11.get(), e10.get(), entityList, invisibleEntityList, frame_delta);
	sidescrollComponent2->setScroll(10);
	e11->addComponent(sidescrollComponent2);
	...
	////Spawn points
	//auto e22 = std::make_unique<GameObject>(600, 400, 50, 50);
	...
	entityList->push_back(std::move(e17));
	entityList->push_back(std::move(e18));
	entityList->push_back(std::move(e19));
	entityList->push_back(std::move(e20));
	entityList->push_back(std::move(e21));
	entityList->push_back(std::move(e22));
	entityList->push_back(std::move(e23));
	

	invisibleEntityList->push_back(std::move(e10));
	invisibleEntityList->push_back(std::move(e11));
	turn = 0;
	playerWin = false;
	return manager.get();
}
