...
void processMessages(std::string messageFromClient, std::vector<std::unique_ptr<GameObject>>* entityList, int64_t frame_delta, Timeline* t, EventManager* m) {
    ...
    if (foundIt == activeClientList.end()) {
        ...
        auto newEntity = std::make_unique<GameObject>(1000, 200, 50, 100, clientID, true, true);
        newEntity->setPlayerNum(2);
        ...
        auto newEntity2 = std::make_unique<GameObject>(1000, 800, 50, 100, clientID, true, true);
        newEntity2->setPlayerNum(1);
        newEntity2->addComponent(new DrawComponent(newEntity2.get(), 0x00, 0xff, 0x00));
        newEntity2->addComponent(new ControllableComponent(newEntity2.get(), entityList, &frame_delta, getManager()));
        newEntity2->setManager(m);
        entityList->push_back(std::move(newEntity2));
